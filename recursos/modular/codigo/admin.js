...
function realizarRegalo(event){
	cargando()
    event.preventDefault()
    envio={
		numero:valor('numero_tarjeta'), 
		correo: valor('correo_tarjetas'), 
		fecha: new Date(), 
		tarjeta: valor('elecc_trj')
	}
	$.ajax({
		method: "POST",
		url: "/admin-tarjetas/regalar",
		data: envio
	}).done(( respuesta )=>{
		no_cargando()
		if(respuesta == 'Error')swal('Ha ocurrido un error, intente nuevamente')
		else if(respuesta == 'Error 1')swal('Esta tarjeta está asignada a un vendedor o ya ha sido vendida, no se puede regalar')
		else swal('Tarjeta regalada con éxito').then(()=>{
			emailjs.send("default_service","template_KK3G9LwJ",{
				to_name: "Usuario",
				to_destinatario: respuesta,
				mensaje: `El administrador de EcuadorActiva le ha otrogado a usted una tarjeta preactivada gratuitamente, inicie sesión para revisar`
		  }
		  ).then(
				(response)=> {
					 no_cargando()   
					 swal("Listo", "Se ha notificado al beneficiario por email")
					 location.reload()
				}, 
				(error)=> {
					 swal("Error", "Se concretó el proceso pero no se notificó al beneficiario")	 
					 location.reload()
				}
		  );
		})
